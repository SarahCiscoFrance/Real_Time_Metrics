...
//data
    //get temp
    xapi.status.get("RoomAnalytics AmbientTemperature").then((temp) => {
    ...
    //get humid
    xapi.status.get("RoomAnalytics RelativeHumidity").then((humid) => {
    ...
    //get air PPB
    xapi.status.get("RoomAnalytics AirQuality TVOCppb").then((airq) => {
        console.log("this is the ppb: "+airq);
        tvoc = airq;
    });

    //get air stat
    xapi.status.get("RoomAnalytics AirQuality Status").then((airstat) => {
